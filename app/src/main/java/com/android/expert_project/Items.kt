...
object Items {
    var itemList = mutableListOf<Item>(
        ...
        Item(
            R.drawable.sample11,
            "27000 스타벅스 디즈니 데비텀블러 새것 1개 정가이하",
            "교환환불불가\n" + "흥정불가\n" + "새것" + "흥정시 차단",
            "유니",
            20000,
            "부산시 동래구 명륜동",
    ...
    fun findByUser(user: String): Item {
        return itemList.find { it.userName == user } ?: Item()
    }

    fun getIndex(idx: Int): Item {
        return itemList[idx]
    }

    fun removeUser(user: String) {
        itemList.remove(itemList.find { it.userName == user })
    }

    fun plusHeart(user: String) {
        findByUser(user).heartCount++
    }

    fun minusHeart(user: String) {
        findByUser(user).heartCount--
    }
}
